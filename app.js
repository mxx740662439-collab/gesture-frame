...
function getDecodeErrorMessage(file) {
  const extension = getVideoExtension(file.name);
  const isAppleContainer = ["mov", "m4v", "hevc", "h265"].includes(extension);
  const format = extension ? `.${extension.toUpperCase()}` : "该";

  if (isAppleContainer) {
    return `${format} 视频可能使用 HEVC/H.265 编码。请在 iPhone、iPad 或 Mac 上使用最新版 Safari 打开；其他浏览器可能无法解码 HEVC。`;
  }

  return `无法解码 ${format} 视频。建议使用 MP4/MOV 容器，并采用 H.264 编码；HEVC/H.265 请优先使用 Safari。`;
}
...
function loadVideo(file) {
  ...
  video.onerror = () => {
    ...
    console.error("视频解码失败", video.error, { name: file.name, type: file.type });
    failOnce(getDecodeErrorMessage(file));
  };
  ...
  video.onloadedmetadata = () => {
    ...
    if (video.duration > MAX_VIDEO_SECONDS) {
      failOnce(`视频时长为 ${video.duration.toFixed(1)} 秒，请选择 10 秒以内的视频`);
      return;
    }
    ...
    extractFrames(runId).catch(error => {
      if (runId !== extractionId) return;
      console.error(error);
      failOnce(`${getDecodeErrorMessage(file)} 如果视频可以播放但无法拆帧，请将“相机 → 格式”暂时设为“兼容性最佳”后重新拍摄。`);
    });
  };
